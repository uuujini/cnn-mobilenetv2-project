...
def conv_block(inputs, filters, kernel_size, strides=1, use_dropout=False, use_batchnorm=True):
...
def depthwise_conv_block(inputs, depthwise_filter, pointwise_filter, strides=1, use_dropout=False, use_batchnorm=True):
    ...
    if use_batchnorm:
        depthwise_conv = tf.nn.batch_normalization(depthwise_conv, mean=0, variance=1, offset=0, scale=1, variance_epsilon=0.001)
    ...
    if use_batchnorm:
        pointwise_conv = tf.nn.batch_normalization(pointwise_conv, mean=0, variance=1, offset=0, scale=1, variance_epsilon=0.001)
    if use_dropout:
...
def build_custom_model(input_shape, num_classes):
    ...
    x = conv_block(inputs, filters, kernel_size=3, strides=2, use_dropout=True)

    # Depthwise separable convolution blocks
    depthwise_filter = tf.Variable(tf.random.normal([3, 3, 32, 1]))
    pointwise_filter = tf.Variable(tf.random.normal([1, 1, 32, 64]))
    x = depthwise_conv_block(x, depthwise_filter, pointwise_filter, strides=1, use_dropout=True)

    depthwise_filter = tf.Variable(tf.random.normal([3, 3, 64, 1]))
    pointwise_filter = tf.Variable(tf.random.normal([1, 1, 64, 128]))
    x = depthwise_conv_block(x, depthwise_filter, pointwise_filter, strides=2, use_dropout=True)
...
def build_and_train_model():
    ...
    # Log file
    log_file_path = 'mobilenetv2_model_training_log.txt'
    ...
    tf.saved_model.save(model, 'trained_model_mobilenetv2')
