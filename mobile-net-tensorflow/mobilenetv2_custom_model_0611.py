...
def save_weights_to_h5(model, filepath):
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    with h5py.File(filepath, 'w') as f:
        for var in model.trainable_variables:
            dataset_name = var.name.replace(':', '_')
            f.create_dataset(dataset_name, data=var.numpy())

def load_weights_from_h5(model, filepath):
    with h5py.File(filepath, 'r') as f:
        for var in model.trainable_variables:
            dataset_name = var.name.replace(':', '_')
            var.assign(f[dataset_name][:])
...
def build_and_train_model():
    ...
    log_file_path = 'custom_training_log_third.txt'
    ...
    for epoch in range(1):
        ...
        val_loss_total = val_loss_total / len(validation_data)
        val_accuracy_metric.reset_states()

        print(f"Epoch {epoch + 1}, Validation Accuracy: {val_accuracy:.4f}, Validation Loss: {val_loss_total:.4f}")

        with open(log_file_path, 'a') as log_file:
            log_file.write(f"{epoch + 1},{step},{avg_loss:.4f},{avg_accuracy:.4f},{val_accuracy:.4f},{val_loss_total:.4f}\n")

        # Early stopping
        if val_loss_total < best_val_loss:
            best_val_loss = val_loss_total
            wait = 0
            save_weights_to_h5(model, 'model_checkpoints/0611/best_model.h5')
            print(f"Model improved, saving model to model_checkpoints/0611/best_model.h5")
        else:
            wait += 1
            if wait >= patience:
    ...
    save_weights_to_h5(model, 'model_checkpoints/mobilenetv2_custom_model_0611.h5')
